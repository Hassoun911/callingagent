...
export default function CompanyPortalControlled({ user }: { user: AuthUser }) {
  ...
  return <div className="flex h-dvh overflow-hidden bg-background text-foreground">{mobileOpen && <button className="fixed inset-0 z-40 bg-black/75 lg:hidden" onClick={() => setMobileOpen(false)} />}
    ...
    <div className="flex min-w-0 flex-1 flex-col overflow-hidden"><header className="flex h-16 items-center border-b border-border px-4 sm:px-6"><button onClick={() => setMobileOpen(true)} className="mr-3 h-10 w-10 rounded-xl border border-border lg:hidden"><Menu className="mx-auto h-5 w-5" /></button><div><p className="text-sm font-semibold">Company Portal</p><p className="text-[11px] text-muted-foreground">{companyName}</p></div><div className="ml-auto flex items-center gap-2 rounded-full border border-emerald-500/20 px-3 py-1.5 text-xs text-emerald-300"><Wifi className="h-3.5 w-3.5" />Live{liveStats.activeCalls > 0 && <><Radio className="h-3 w-3 animate-pulse" />{liveStats.activeCalls}</>}</div></header>
