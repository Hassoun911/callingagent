            ...
            <section key={group.label} aria-labelledby={`portal-nav-${group.label.toLowerCase().replace(/\s+/g, "-")}`}>
              ...
              <div className="space-y-1">
                {group.items.map(item => {
                  ...
                  const attentionAlert = (item.badgeKey === "newBookings" || item.badgeKey === "recentCalls") && badge > 0;
                  const linkClass = attentionAlert
                  ...
                  const iconClass = attentionAlert
                  ...
                  return (
                    ...
                    >
                      <span className={`absolute inset-y-2 left-0 w-0.5 rounded-r-full transition-opacity ${attentionAlert ? "bg-red-400 opacity-100" : `bg-cyan-400 ${active ? "opacity-100" : "opacity-0"}`}`} />
                      <Icon className={`shrink-0 transition-colors ${iconClass}`} strokeWidth={attentionAlert ? 2.2 : 1.8} />
                      <span className={`truncate ${attentionAlert ? "text-[16px] font-extrabold tracking-[0.01em]" : "text-sm"}`}>{item.label}</span>
                      {badge > 0 && (
                        <span className={`ml-auto rounded-full text-center font-black ${attentionAlert
